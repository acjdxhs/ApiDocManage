...
import com.scut.originsystem.entity.QRCodeBoughtForCheck;
...
//        return "good";
    ...
    @RequestMapping(value = "/getMerchantDetailList",method = RequestMethod.GET)
    @ResponseBody
    public Object getMerchantDetailList(
            @RequestParam(value="page",required = false,defaultValue = "0")int page){
        return merchantService.getMerchantDetailList(page);
    }
    ...
    public Object getAllInfo(HttpServletRequest request){
        return merchantService.getAllInfo(request);
    }
    ...
    //二维码购买申请
    @RequestMapping(value = "/applyForQRCode",method = RequestMethod.POST)
    @ResponseBody
    @SystemLogControllerAnno(description = "申请购买二维码")
    public Object applyForQRCode(@Validated @RequestBody QRCodeBoughtForCheck qrCodeBoughtForCheck){
        return merchantService.buyQRCode(qrCodeBoughtForCheck);
    }

    //查看我的二维码购买审核表
    @RequestMapping(value = "/getMyBoughtCheck",method = RequestMethod.GET)
    @ResponseBody
    public Object getMyBoughtCheck(@RequestParam(value = "merchant_id") int merchant_id,
                             @RequestParam(value = "status", required = false, defaultValue = "0") int status,
                             @RequestParam(value = "page", required = false, defaultValue = "1") int page){
        return merchantService.getMyBoughtCheck(merchant_id,status,page);
    }

    //商户剩余二维码数量查询
    @RequestMapping(value = "getMerchantQRCode_left",method = RequestMethod.GET)
    @ResponseBody
    public Object getMerchantQRCode_left(@RequestParam(value = "merchant_id",required = true) int merchant_id){
        return merchantService.getMerchantQRCode_left(merchant_id);
    }

    //商户已用二维码数量查询
    @RequestMapping(value = "getMerchantQRCode_used",method = RequestMethod.GET)
    @ResponseBody
    public Object getMerchantQRCode_used(@RequestParam(value = "merchant_id",required = true) int merchant_id){
        return merchantService.getMerchantQRCode_used(merchant_id);
    }
